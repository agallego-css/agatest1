...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'agatest1.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'agatest1.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'agatest1.Client.ProtectedToken'
